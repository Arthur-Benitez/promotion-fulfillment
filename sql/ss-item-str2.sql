...
SSG AS
  ...
  FROM
    MX_CF_VM.ITEM T1
  INNER JOIN MX_CF_REPL_VM.GRS_FCST_DEMAND T3 ON T1.ITEM_NBR = T3.ITEM_NBR
  WHERE
    T3.FCST_DATE = DATE),
...
SST AS
  ...
  FROM
    MX_CF_VM.ITEM T1
  INNER JOIN MX_CF_REPL_VM.GRS_PRESENTATION_PARM T2 ON T1.ITEM_NBR = T2.ITEM_NBR
  WHERE
...
MAX_SS AS
  ...
  FROM
    MX_CF_VM.ITEM T1
  INNER JOIN MX_CF_REPL_VM.GRS_FULFILLMENT_PARM T2 ON T1.ITEM_NBR = T2.ITEM_NBR),
...
SSC AS
  ...
  LEFT JOIN
    ...
    ON (A.SS_EVENT_NAME = B.SS_EVENT_NAME AND A.MAX_EFF = B.SS_EVENT_EFF_DATE))T2
