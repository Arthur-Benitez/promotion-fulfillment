...
FULF AS (
  ...
  FROM MX_CF_REPL_VM.GRS_FCST_DEMAND AS G
  	...
  	LEFT JOIN EVENT
		ON G.SS_EVENT_NAME = EVENT.SS_EVENT_NAME
	INNER JOIN ITEMS
		ON G.ITEM_NBR = ITEMS.ITEM_NBR
...
FROM
	FULF
	LEFT  JOIN BPR ON    FULF.ITEM_NBR = BPR.ITEM_NBR    AND FULF.STORE_NBR = BPR.STORE_NBR
	LEFT  JOIN FCST ON   FULF.ITEM_NBR = FCST.ITEM_NBR   AND FULF.STORE_NBR = FCST.STORE_NBR
	INNER JOIN STORES ON                                    FULF.STORE_NBR = STORES.STORE_NBR
