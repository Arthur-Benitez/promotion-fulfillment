SELECT
A.*,
CAST(ZEROIFNULL(SS_Press)+ZEROIFNULL(BASE_PRESS) AS  DECIMAL(19,2 )) AS SS_PRESS_Tot ,
CASE 
...
SUM(ZEROIFNULL(CAST(SST.SSPRESS AS DECIMAL(19,2 ))))  AS SS_PRESS,
SUM(ZEROIFNULL(CAST(SST.DISPLAY AS DECIMAL(19,2 ))))  AS DISPLAY,

SUM(ZEROIFNULL(CAST(BPR.BASE_PRESS AS DECIMAL(19,2 )))) AS BASE_PRESS,
SUM(ZEROIFNULL(CAST(SSG.SSGANADOR AS DECIMAL(19,2 )))) AS SS_GANADOR,
SUM(ZEROIFNULL(CAST(MAX_SS.MAXSS  AS DECIMAL(19,2 )))) AS MAX_SS,
SUM(ZEROIFNULL(CAST(MAX_SS.MINSS  AS DECIMAL(19,2 ))))  AS MIN_SS,

SUM(ZEROIFNULL(CAST(MAX_SS.SSCOV  AS DECIMAL(19,2 )))) AS SSCOV_,
--SUM(ZEROIFNULL(CAST(MAX_SS.SSCOV_INT AS DECIMAL(19,2 )))) AS SSCOV_INT,
--SUM(ZEROIFNULL(CAST(MAX_SS.DEC_SSCOV AS DECIMAL(19,2 )))) AS DEC_SSCOV,

SUM(ZEROIFNULL(CAST(SSC.SSTEMP  AS DECIMAL(19,2 )))) AS SSTEMP_,
--SUM(ZEROIFNULL(CAST(SSC.SSTEMP_INT AS DECIMAL(19,2 )))) AS SSTEMP_INT,
--SUM(ZEROIFNULL(CAST(SSC.DEC_SSTEMP AS DECIMAL(19,2 )))) AS DEC_SSTEMP,

(SSCOV_+SSTEMP_) AS SSCOV_TOT
--(TRUNC(SSCOV_TOT)) AS SSCOV_TOT_INT,
--(SSCOV_TOT-SSCOV_TOT_INT) AS DEC_SSCOV_TOT
...
ON SSG.ITEM_NBR=SST.ITEM_NBR AND SSG.STORE_NBR=SST.STORE_NBR
...
ON SSG.ITEM_NBR=BPR.ITEM_NBR AND SSG.STORE_NBR=BPR.STORE_NBR
...
TRUNC(T2.MIN_SS_SUPPLY_DAY_QTY) AS SSCOV_INT,
SSCOV-SSCOV_INT AS DEC_SSCOV
...
ON  SSG.ITEM_NBR=MAX_SS.ITEM_NBR  AND  SSG.STORE_NBR=MAX_SS.STORE_NBR
...
TRUNC(SSTEMP) AS SSTEMP_INT,
SSTEMP-SSTEMP_INT AS DEC_SSTEMP
...
ON (T1.SS_EVENT_NAME=T2.SS_EVENT_NAME)) SSC

ON  SSG.ITEM_NBR=SSC.ITEM_NBR  AND  SSG.STORE_NBR=SSC.STORE_NBR
