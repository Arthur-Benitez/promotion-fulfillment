...
SELECT
  ...
  ZEROIFNULL(CAST(MAX_SS.SSCOV AS DECIMAL(19, 2))) AS SSCOV_DAY,
  ZEROIFNULL(CAST(SSC.SSTEMP AS DECIMAL(19, 2))) AS SSTEMP_DAY,
  ZEROIFNULL(CAST((SSCOV_DAY + SSTEMP_DAY) AS DECIMAL(19, 2))) AS SSCOV_DAY_TOT,
  ...
  ZEROIFNULL(SSCOV_DAY * AVG_DLY_FCST) AS SSCOV,
  ZEROIFNULL(SSTEMP_DAY * AVG_DLY_FCST) AS SSTEMP,
  ZEROIFNULL(SSCOV_DAY_TOT * AVG_DLY_FCST) AS SSCOV_TOT,
  ...
  CASE
    ...
    WHEN SS_GANADOR BETWEEN (SSCOV - 0.2) AND (SSCOV + 0.2) THEN 'SSTEMP'
    ...
    WHEN SSCOV BETWEEN (SS_GANADOR * 0.9) AND (SS_GANADOR * 1.1) THEN 'SSTEMP'
...
LEFT JOIN STORES ON                                    SSG.STORE_NBR = STORES.STORE_NBR
