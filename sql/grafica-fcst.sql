...
(SELECT
				...
				WHERE
					...
					AND T2.WM_YR_WK >= (SELECT DISTINCT WM_YR_WK -200 FROM MX_CF_VM.CALENDAR_DAY WHERE GREGORIAN_DATE = CURRENT_DATE) -- two years sales history
					AND T2.WM_YR_WK < (SELECT DISTINCT WM_YR_WK FROM MX_CF_VM.CALENDAR_DAY WHERE GREGORIAN_DATE = CURRENT_DATE) --up to last week
