...
TG AS (
  ...
  FROM
    ...
    INNER JOIN MX_CF_VM.STORE_INFO AS T
      ON A.STORE_NBR = T.STORE_NBR
    INNER JOIN NEGO AS N
      ON A.STORE_NBR = N.STORE_NBR
    INNER JOIN XREF
      ON A.ITEM_NBR = XREF.ITEM_NBR
  WHERE
    I.OLD_NBR IN (?OLD_NBRS)
		AND I.STATUS_CODE IN ('A')
...
FCST AS (
  SELECT
    XREF.PRIME_XREF_ITEM_NBR,
		PRE_FCST.STORE_NBR,
		PRE_FCST.WM_YR_WK,
		SUM(PRE_FCST.WK_FCST_QTY) AS WK_FCST_QTY
	FROM XREF
  	INNER JOIN PRE_FCST
    	ON PRE_FCST.ITEM_NBR = XREF.ITEM_NBR
  GROUP BY 1,2,3
),
...
STORE_COUNTS AS (
  ...
  FROM  MX_CF_VM.REPL_SKU_WKLY_INV AS OH
    ...
    INNER JOIN NEGO ON OH.STORE_NBR = NEGO.STORE_NBR
  GROUP BY 1,2,3
)
