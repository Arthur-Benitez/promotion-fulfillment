/*
ESTE QUERY GENERA LA CONSULTA A NIVEL ITST DEL PROMEDIO DE VENTA DE UN PERIODO ESPECIFICO,
...
--=================Tabla General (TG) de combinaciones ITST activas, validas y con inventario de Articulos Resurtibles. Contiene especificaciones del articulo, banderas de validez, inventario en piezas y a costo==============
	...
	(
		...
		AND (T1.STORE_NBR IN (?WHITE_LIST) OR (?CONTROL_W))
		AND (T1.STORE_NBR NOT IN (?BLACK_LIST) OR (?CONTROL_B))
...
--=================Tabla POS. Obtiene el promedio de Venta Diaria de los old_nbr y semanas que se incluyan en los filtros ==============
	...
	(
		...
		FROM
			(
				-- Pre sumarizar para quitar renglones con misma llave (misma combinacion item-store-wk)
				SELECT
