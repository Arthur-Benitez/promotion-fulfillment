...
INV AS (
  ...
  WHERE
  	WKLY_INV.WM_YR_WK >= ('?WK_INICIO')
		AND WKLY_INV.WM_YR_WK <= ('?WK_FINAL')
...
COMB AS (
	...
	FROM
  	...
  	INNER JOIN XREF
	    ON WKLY_INV.ITEM_NBR = XREF.ITEM_NBR
  WHERE
  	WKLY_INV.WM_YR_WK >= ('?WK_INICIO' - 4)
		AND WKLY_INV.WM_YR_WK <= ('?WK_INICIO' - 1)
),
...
SELECT
	...
	FROM (
	  SELECT UNIQUE
			...
			STORE_NBR,
	    WM_YR_WK,
			GREATEST(WKLY_QTY, 0) AS WKLY_QTY
		FROM
			MX_CF_VM.SKU_DLY_POS
		WHERE
			REPORT_CODE NOT IN (8)
			AND WM_YR_WK >= ('?WK_INICIO' - 4)
			AND WM_YR_WK <= ('?WK_INICIO' - 1)
	) SALES
	RIGHT JOIN COMB
...
SELECT
  ...
  INV.OH_QTY,
	POS.SALES_AVG,
	CASE WHEN POS.SALES_AVG <= 0 THEN NULL
		ELSE (INV.OH_QTY * 1.000 / (POS.SALES_AVG / 7.000))
	END AS DDV_OH
FROM
	INV
	LEFT JOIN POS
