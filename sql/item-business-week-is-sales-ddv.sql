	...
	FROM
		...
		LEFT JOIN (
		  SELECT
			...
			ON SALES.PRIME_XREF_ITEM_NBR = INV.PRIME_XREF_ITEM_NBR AND SALES.STORE_NBR = INV.STORE_NBR AND SALES.WM_YR_WK = INV.WM_YR_WK
	GROUP BY 1, 2
),
...
INST AS (
	SELECT
		INST.DEPT_NBR,
		INST.CATEGORY_NBR,
		CAT.DESCRIPCION AS CATEGORY_NAME,
		XREF.PRIME_XREF_ITEM_NBR,
		INST.NEGOCIO,
		INST.WM_YR_WK,
		SUM(INST.CATALOGO) AS CATALOGO,
		SUM(INST.FALTANTES) AS FALT_OH,
		SUM(INST.FALTANTES_TTL) AS FALT_TTL,
		1 - (SUM(INST.FALTANTES) * 1.0000 / SUM(INST.CATALOGO) * 1.0000) AS IS_OH,
		1 - (SUM(INST.FALTANTES_TTL) * 1.0000 / SUM(INST.CATALOGO) * 1.0000) AS IS_TTL	
	FROM
		MR101_WM_AD_HOC.INSTOCK_ITEM AS INST
		INNER JOIN XREF
			ON XREF.ITEM_NBR = INST.ITEM_NBR
		LEFT JOIN WM_AD_HOC.CATEGORIA AS CAT
			ON INST.DEPT_NBR = CAT.DEPTO AND INST.CATEGORY_NBR = CAT.CATEGORIA
	WHERE
		INST.NEGOCIO IN (?NEGOCIOS)
...
FROM
	OH_POS
	LEFT JOIN INST
		ON INST.PRIME_XREF_ITEM_NBR = OH_POS.PRIME_XREF_ITEM_NBR AND INST.NEGOCIO = OH_POS.NEGOCIO AND INST.WM_YR_WK = OH_POS.WM_YR_WK
ORDER BY DEPT_NBR, CATEGORY_NBR, CATEGORY_NAME, REPL_GROUP_NBR, INST.PRIME_XREF_ITEM_NBR, FORMATO, INST.WM_YR_WK
