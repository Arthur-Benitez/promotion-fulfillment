...
OH_POS AS (
	SELECT
	  ...
	  BASE.CATEGORY,		
		...
		PERCENTILE_CONT(0.00) WITHIN GROUP (ORDER BY BASE.DDV_OH_ST) AS P000,
		PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY BASE.DDV_OH_ST) AS P025,
		PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY BASE.DDV_OH_ST) AS P050,
		PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY BASE.DDV_OH_ST) AS P075,
		PERCENTILE_CONT(1.00) WITHIN GROUP (ORDER BY BASE.DDV_OH_ST) AS P100
	FROM
		BASE
	LEFT JOIN POS_STUD
...
FROM
	INST
	LEFT JOIN PERC_SUM
		ON INST.PRIME_XREF_ITEM_NBR = PERC_SUM.PRIME_XREF_ITEM_NBR AND INST.FORMATO = PERC_SUM.NEGOCIO AND INST.WM_YR_WK = PERC_SUM.WM_YR_WK
	LEFT JOIN OH_POS
		ON INST.PRIME_XREF_ITEM_NBR = OH_POS.PRIME_XREF_ITEM_NBR AND INST.FORMATO = OH_POS.NEGOCIO AND PERC_SUM.WM_YR_WK = OH_POS.WM_YR_WK
