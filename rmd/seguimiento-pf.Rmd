...
metrics_info <- res$is %>% 
  left_join(
    res$sales,
    by = c(
      'prime_xref_item_nbr' = 'prime_xref_item_nbr',
      'formato' = 'negocio',
      'wm_yr_wk' = 'wm_yr_wk'
    )
  ) %>% 
...
inv_info <- res$stats %>% 
  left_join(
    res$sales %>% 
      select(-item1_desc),
    by = c('prime_xref_item_nbr', 'negocio', 'wm_yr_wk')
  ) %>% 
  left_join(
    res$is,
    by = c(
...
## Preparación de info por categoría-negocio
inv_sum <- inv_info %>% 
  group_by(dep_nbr, category_nbr, category_name, negocio, wm_yr_wk) %>% 
  summarise(
    wkly_qty = sum(wkly_qty),
    oh = sum(oh),
    sales = sum(sales),
...
graph_inv_sum <- inv_sum %>% 
  group_by(dep_nbr, category_nbr, category_name, negocio) %>% 
  group_map(~inv_plot(.x, .y))
...
## Preparación de info por categoría-negocio
box_sum <- box_info  %>%
  group_by(category, category_name, negocio, wm_yr_wk) %>%
  summarise(
    oh = sum(oh),
    sales = sum(sales),
...
```{r inv_ind, echo=FALSE, fig.height=8, fig.width=19, message=FALSE, warning=FALSE}
grids_inv_ind
```
...
```{r inv_sum, echo=FALSE, fig.height=8, fig.width=19, message=FALSE, warning=FALSE}
grids_inv_sum
```
