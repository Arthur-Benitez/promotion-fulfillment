...
bins_plotly <- function(data, keys){
  data %>% 
    ...
    layout(
      annotations = list(
        x = ifelse(keys$group_id %% 2 == 1, 0.35, 0.65),
        y = 1,
        text = apply(
...
graph_ddv_sum <- bins_info  %>%
  ...
  group_by(category, negocio, group_id) %>% 
  complete(
    wm_yr_wk,
    bin = bines,
    fill = list(
...
wmwk1 <- min(metrics$wm_yr_wk) 
wmwk2 <- max(metrics$wm_yr_wk)
...
pretty_names <- c(articulo = 'item_nbr', descripcion = 'item1_desc.x')
...
summary <- metrics %>% 
  ...
  spread(temp, value) %>% 
  mutate(
    is_oh_dif = (!!is_oh_2 - !!is_oh_1) / !!is_oh_1,
    ddv_oh_dif = (!!ddv_oh_2 - !!ddv_oh_1) / !!ddv_oh_1,
    wkly_qty_dif = (!!wkly_qty2 - !!wkly_qty1) / !!wkly_qty1
  ) %>% 
  select(
    # category_nbr,
    item_nbr,
...
summary_table <- summary %>% 
  mutate_at(vars(one_of(c(as.character(is_oh_1), as.character(is_oh_2)))), scales::percent, accuracy = 0.1) %>%
  mutate_at(vars(one_of(c(as.character(wkly_qty1), as.character(wkly_qty2)))), scales::comma) %>% 
  mutate_at(vars(one_of(c(as.character(ddv_oh_1), as.character(ddv_oh_2)))), scales::comma, accuracy = 0.1) %>% 
  mutate_at(vars(ends_with('dif')), scales::percent, accuracy = 0.01) %>%
  mutate(
    is_oh_dif = cell_spec(is_oh_dif, 'html', color = ifelse(is_oh_dif < 0, 'red', ifelse(is_oh_dif > 0, '#35a15c', 'black'))),
    ddv_oh_dif = cell_spec(ddv_oh_dif, 'html', color = ifelse(ddv_oh_dif < 0, '#35a15c', ifelse(ddv_oh_dif > 0, 'red', 'black'))),
    wkly_qty_dif = cell_spec(wkly_qty_dif, 'html', color = ifelse(wkly_qty_dif < 0, 'red', ifelse(wkly_qty_dif > 0, '#35a15c', 'black')))
  ) %>% 
...
## Dispersión - Boxplot - Categoría-Formato
...
## Dispersión - Bines - Categoría-Formato
...
Esta tabla muestra la evolución de los indicadores clave entre la primera y última semana del estudio.
