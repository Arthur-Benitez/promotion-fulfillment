...
re_order <- function(data, cat_flag){
  if(cat_flag){
    data %>%
      data.table::setorder(
        dep_nbr,
...
group_vars_item <- c('prime_xref_item_nbr', 'formato', 'repl_group_nbr', 'item1_desc', 'dep_nbr', 'category_nbr', 'category_name', 'group_id')

## Preparación de info por item-negocio
graph_ddv_ind <- bins_info  %>% 
  mutate(group_id = group_indices(., prime_xref_item_nbr, item1_desc, formato)) %>% 
  group_by(!!!syms(group_vars_item)) %>% 
  complete(
    wm_yr_wk,
    bin = bines,
    fill = list(
...
group_vars_cat <- c('dep_nbr', 'category_nbr', 'category_name', 'formato')
