...
```{r is_graph, echo=FALSE, message=FALSE, warning=FALSE}
metrics <- res$is %>% 
  left_join(
    res$sales %>% 
...
graph_metrics <- metrics %>% 
  ...
  group_map(~{
    .x %>% 
      ...
      labs(
        title = sprintf('Indicadores durante la estrategia - %s - %s', .y$item_nbr, .y$formato),
        x = 'Semanas Walmart',
        y = 'Porcentaje (%)',
        colour = 'Indicador',
        caption = sprintf('Indicadores calculados al cierre de las semanas. Item: %s.', .y$primary_desc)
      ) + 
      theme(
...
grids_metrics <- gridExtra::marrangeGrob(
  ...
  nrow = 2,
  ncol = 2,
  top =  quote(paste('Resumen', g, 'of', npages))
)
...
```{r box_plot, echo=FALSE, message=FALSE, warning=FALSE}
box_info <- res$stats  %>% 
  mutate(
...
graph_box <- box_info  %>% 
  ...
  group_map(~{
    .x %>% 
      ...
      labs(
        title = 'Dispersión durante la estrategia',
        x = 'Semanas Walmart',
        y = 'DDV'
      ) + 
      theme(
