...
## UI
user_managementUI <- function(id) {
  ns <- shiny::NS(id)
  shiny::uiOutput(ns('user_management'))
}

## Server
user_managementServer <- function(input, output, session, credentials) {
  output$user_management <- shiny::renderUI({
    ns <- session$ns
